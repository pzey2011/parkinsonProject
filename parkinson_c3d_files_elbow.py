...
def load_data():
    ...
    target_c3d_column_keyword = 'Elbow'
    ...
    for csv_c3d_file_path in glob.glob(os.path.join(input_c3d_directory, '**', '*.csv'), recursive=True):
        ...
        if any(walk in input_csv_c3d_filename for walk in ['walk_1_angular_kinematics','walk_2_angular_kinematics','walk_3_angular_kinematics']):
            try:
                ...
                elbow_columns = [col for col in input_c3d_df.columns if target_c3d_column_keyword in col]
                if elbow_columns:
                    input_elbow_c3d_data = input_c3d_df[elbow_columns]
                    input_elbow_c3d_data = input_elbow_c3d_data.drop(input_elbow_c3d_data.index[0])
            ...
            try:
                ...
                if c3d_input_related_target is not None:
                    ...
                    if target_column and target_column in c3d_input_related_target.columns:
                        ...
                        c3d_input_related_target_series = pd.Series([c3d_input_related_target_data] * len(input_elbow_c3d_data))
                        c3d_X = input_elbow_c3d_data
