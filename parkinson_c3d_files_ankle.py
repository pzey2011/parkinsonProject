...
def load_data():
    # Add dictionaries to store file names and their corresponding target values
    ...
    target_c3d_column_keyword = 'Ankle'
    ...
    for csv_c3d_file_path in glob.glob(os.path.join(input_c3d_directory, '**', '*.csv'), recursive=True):
        ...
        if any(walk in input_csv_c3d_filename for walk in ['walk_1_angular_kinematics','walk_2_angular_kinematics','walk_3_angular_kinematics']):
            try:
                ...
                ankle_columns = [col for col in input_c3d_df.columns if target_c3d_column_keyword in col]
                if ankle_columns:
                    input_ankle_c3d_data = input_c3d_df[ankle_columns]
                    input_ankle_c3d_data = input_ankle_c3d_data.drop(input_ankle_c3d_data.index[0])
            ...
            try:
                ...
                if c3d_input_related_target is not None:
                    ...
                    if target_column and target_column in c3d_input_related_target.columns:
                        ...
                        c3d_input_related_target_series = pd.Series([c3d_input_related_target_data] * len(input_ankle_c3d_data))
                        c3d_X = input_ankle_c3d_data
...
def print_distribution(data):
    input_related_target_series_as_int = data.astype(int)
...
def print_distribution_series(data):
    # Convert the numpy array to a pandas Series
    input_related_target_series_as_int = pd.Series(data.astype(int))

    # Calculate the distribution of class labels
    distribution = input_related_target_series_as_int.value_counts(sort=True)

    print("Distribution of class labels: series")
    print(distribution)

    # To get the distribution in proportions, you can do:
    distribution_normalized = input_related_target_series_as_int.value_counts(normalize=True, sort=True)

    distribution_percent = distribution_normalized * 100
    print("Distribution of class labels in percent: series")
    print(distribution_percent)
def plot_confusion_matrix(y_pred, X_test, y_test):
    # Creating the confusion matrix
    cm = confusion_matrix(y_test, y_pred)

    # Plotting the confusion matrix with larger font sizes
    plt.figure(figsize=(10, 7))
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", cbar=False, annot_kws={"size": 12})  # Set annotation font size
    plt.ylabel("Actual Hoehn & Yahr Score", fontsize=12)  # Set y-axis label font size
    plt.xlabel("Predicted Hoehn & Yahr Score", fontsize=12)  # Set x-axis label font size
...
def cm_displayer(cm):
    ...
    plt.ylabel("Actual Hoehn & Yahr Score")
    plt.xlabel("Predicted Hoehn & Yahr Score")
    plt.show()
...
def svm_optimization(X_train_scaled_c3d, X_test_scaled_c3d, y_train_c3d, y_test_c3d):
    ...
    y_pred = svm_classifier_c3d.predict(X_test_scaled_c3d)
...
def decision_tree_optimization(X_train_scaled_c3d, X_test_scaled_c3d, y_train_c3d, y_test_c3d):
    ...
    def objective(trial):
        # Expanding max_features search options
        ...
        if max_features_option == 'int':
            ...
            max_features = trial.suggest_float('max_features_float', 0.01, 1.0, step=0.01)
        else:
            max_features = max_features_option

        # Handling min_samples_split with different names for int and float
        if trial.suggest_categorical('min_samples_split_option', ['int', 'float']) == 'int':
            min_samples_split = trial.suggest_int('min_samples_split_int', 2, 50)
        else:
            min_samples_split = trial.suggest_float('min_samples_split_float', 0.01, 1.0, step=0.01)

        # Handling min_samples_leaf with different names for int and float
        if trial.suggest_categorical('min_samples_leaf_option', ['int', 'float']) == 'int':
            min_samples_leaf = trial.suggest_int('min_samples_leaf_int', 1, 50)
        else:
            min_samples_leaf = trial.suggest_float('min_samples_leaf_float', 0.01, 0.5, step=0.01)

        # Expanded parameter search for Decision Tree
        param = {
            'criterion': trial.suggest_categorical('criterion', ['gini', 'entropy', 'log_loss']),
            'splitter': trial.suggest_categorical('splitter', ['best', 'random']),
            'max_depth': trial.suggest_int('max_depth', 1, 128, log=True),
            'min_samples_split': min_samples_split,  # Use the chosen min_samples_split
            'min_samples_leaf': min_samples_leaf,  # Use the chosen min_samples_leaf
            ...
            'min_impurity_decrease': trial.suggest_float('min_impurity_decrease', 0, 0.5, step=0.01),
            'class_weight': trial.suggest_categorical('class_weight', [None, 'balanced']),
            'ccp_alpha': trial.suggest_float('ccp_alpha', 0, 0.5, step=0.01)
        }

        # Model initialization and cross-validation scoring
        clf = DecisionTreeClassifier(**param)
        score = cross_val_score(clf, X_train_scaled_c3d, y_train_c3d, n_jobs=-1, cv=StratifiedKFold(10),
                                scoring='f1_weighted', error_score='raise').mean()

        return score

    # Optuna study for hyperparameter optimization
    study = optuna.create_study(direction='maximize', study_name="Wide Decision Tree Optimization")
    ...
    # Remove unnecessary 'max_features_option' if present
    best_params.pop('max_features_option', None)
    best_params.pop('max_features_int',None)
    best_params.pop('max_features_float',None)
    best_params.pop('min_samples_split_option', None)
    best_params.pop('min_samples_leaf_option', None)
    best_params.pop('min_samples_split_int', None)
    best_params.pop('min_samples_split_float',None)
    best_params.pop('min_samples_leaf_float',None)
    best_params.pop('min_samples_leaf_int', None)
    ...
    print('print distribution of y_train_c3d')
    print_distribution_series(y_train_c3d)
    print('print distribution of y_test_c3d')
    print_distribution_series(y_test_c3d)
    print('print distribution of y_pred')
    print_distribution_series(y_pred)
...
def lgbm_optimization(X_train_scaled_c3d, X_test_scaled_c3d, y_train_c3d, y_test_c3d):
    ...
    y_pred = lgbm_classifier.predict(X_test_scaled_c3d)
...
def xgboost_optimization(X_train_scaled_c3d, X_test_scaled_c3d, y_train_c3d, y_test_c3d):
    ...
    y_pred = xgb_clf.predict(X_test_scaled_c3d)

    # Decode the predictions to original labels
    y_pred_decoded = label_encoder.inverse_transform(y_pred)

    # Calculate F1 score
    f1 = f1_score(y_test_c3d, y_pred_decoded, average='weighted')
    print(f"F1 Score: {f1}")
...
def random_forest_optimization(X_train_scaled_c3d, X_test_scaled_c3d, y_train_c3d, y_test_c3d):
    ...
    y_pred = rf_classifier.predict(X_test_scaled_c3d)
